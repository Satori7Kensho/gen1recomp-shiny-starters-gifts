...
**v1.3.0 adds FireRed/LeafGreen support and independent options for starters, gifts, and NPC trades.**
...
- **SHINY STARTERS** – Makes your starting Pokémon from Oak’s or Elm’s lab shiny.
...
**Each option controls only its own category.** All three default to ON, and you can turn each one on or off independently.

Yellow’s later Bulbasaur, Charmander, and Squirtle gifts use **SHINY GIFTS**. Your starting Pikachu uses **SHINY STARTERS**.
...
3. Extract the `shiny_starters_gifts` folder into your Gen1Recomp `mods/` folder. When updating, replace the previous copy.
...
The mod applies to starters, scripted gifts, and in-game NPC trades. It does not change:
